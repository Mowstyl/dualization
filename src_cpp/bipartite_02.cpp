...
bipartite_02::bipartite_02(const vector<vector<int>>& H)
    ...
    // Constructor that initializes the bipartite graph with the given hypergraph
    // Edges_copy is initialized in the initializer list
}
// This constructor initializes the bipartite graph with the hypergraph edges
// and can be used to create a bipartite_02 object from a hypergraph representation     
// This is useful for algorithms that require a bipartite representation of the hypergraph

bipartite_02::bipartite_02(const bipartite_02& other)
    : bipartite_graph(other), Edges_copy(other.Edges_copy) {
    // Copy constructor that initializes the bipartite graph from another bipartite_02 object
...
bool bipartite_02::is_uncovering(const vector<int>& ce) const {
    ...
    that is contained in the counter example. To work correctly, the counter example
    must be sorted. Also the set of hyperedges must be sorted by length 
    ...
    */
    ...
    for (vector <int>  e : *Edges_copy){  
        /* Since the hyperedges in Edges_copy are sorted by length, if the counter example is smaller than the hyperedge e, 
          e and all othere subsequent hyperedges cannot be contained in it
        */
        if (ls < e.size()) {
            return true; 
        }
...
bipartite_02::compute_H_2(const vector<int>& p, int e_id) {
    /*
        ...
        is computed. Then $H_2 = H_1 \setminus N_{H_1}(s)$ is computed, where $s$ is the edge
        with index e_id. The function returns the backup of the edges, adjacency 
        lists and vertices before the deletion.
...
bool bipartite_02::find_uncovering_hitting_set(int level) {
    ...
    if (Edges.size() == 1) {
        //if there is only one edge, we can use only one vertex at a time from it
    ...
    if (level == 0) {
        ...
        min_iter = min_len - min_level; // Use the minimum level found in the previous recursion
        if (min_iter <0) {
            min_iter = 0; // Ensure we don't have negative iterations
        }
    }
    */
    ...
    for (int i = 1; i <=min_iter; ++i) {
        for( CombinationGenerator<int> gen(elements_for_combinations, i);!gen.is_done();){
            ...
            /*
            if (level == 0) {
                min_level = j; // Set the minimum level for the successive recursion levels
            }
            */
                        
            p = set_difference(E_current_edge, e_p);
            counter_example.insert(counter_example.end(), e_p.begin(), e_p.end());
            ...
            if (n_r) return true; // Found an uncovering hitting set
            ...
            counter_example.erase(counter_example.end() - e_p.size(), counter_example.end());
        }
    }
    return false;
}
